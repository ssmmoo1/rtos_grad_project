...
/********* Prep 2.1 Question #1 *********
This task is fixed bandwidth because it is limited by the requirement to sample at 2kHz (not software or hardware).
****************************************/
...
/********* Prep 2.1 Question #1 *********
This task is I/O Bound because the SSI busy waits in the ST7735 Driver are the limiting factor.
****************************************/
...
void ButtonWork(void){
  ...
  ST7735_Message(1,1,"PIDWork     =",PIDWork, true);
...
/********* Prep 2.1 Question #1 *********
This task is fixed bandwidth because it is limited by the requirement to sample at 400Hz (not software or hardware).
****************************************/
...
/********* Prep 2.1 Question #1 *********
This task is CPU bound because it is limited by how quickly the software can execute the PID control.
****************************************/
...
/********* Prep 2.1 Question #1 *********
This task is I/O bound because it is limited by the speed of the user inputs over UART.
****************************************/
...
// function for dumping hard fault registers

void hardfaultDump(void) {
	LaunchPad_Init();
	UART_Init();
	static char arr[100];
	//sprintf(arr, "CFSR: 0x%.8x\r\n", *((uint32_t *) 0xE000ED28));
	//UART_OutString(arr);
	sprintf(arr, "BFAR: 0x%.x\r\n", *((uint32_t *) 0xE000ED38));
	UART_OutString(arr);
	PF1 ^= 2;
	while(1);
}
